...
exports.run = async (client, message, args) => {
  ...
  if (!args[0])
    return message.channel.send(new Discord.MessageEmbed().setColor("RANDOM").setDescription(`Lütfen g!pre kullan, kontrol veya kodum Yazın`));

  if (args[0] == "ver"  || args[0] == "oluştur") {
    const yetki = new Discord.MessageEmbed()
      .setColor("RANDOM")
      .setDescription(`Sen Kurucum Değilsin!`);
    if (message.author.id !== "786122687218581555")
