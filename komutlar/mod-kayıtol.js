...
if(!sistem || sistem == null || sistem == undefined || sistem == 'kapat') return;
  ...
  if(!isim) return message.channel.send('Seni Kayıt Etmem İçin Bir İsim Girmelisin : `g!kayıtol Mustafa 17`')
if(!yas) return message.channel.send('Yaş İçin En Az 1 Basamak Belirtebilirsin. Örnek: `g!kayıtol Mustafa 17`')
  if(yas.length >= 3) return message.channel.send('Yaş İçin En Fazla 2 Basamak Belirtebilirsin. Örnek: `-kayıtol Mustafa 17`')
  if(isNaN(yas)) return message.channel.send('Yaş İçin Sayı Girmelisin. Örnek: `g!kayıtol Mustafa 17`')
